...
if(isset($_POST['save'])){
	...
	if(empty($kode) or empty($nama)){
		...
		if($action=='add'){
			if(mysql_num_rows(mysql_query("select * from pilihan where kode='".$kode."'"))>0){
				$error='Kode sudah terdaftar. Silahkan gunakan kode yang lain.';
			}else{
				$q="insert into pilihan(kode, nama) values('".$kode."', '".$nama."')";
				mysql_query($q);
				exit("<script>location.href='".$link_list."';</script>");
			}
		}
		if($action=='edit'){
			$q=mysql_query("select * from pilihan where id_pilihan='".$id."'");
			$h=mysql_fetch_array($q);
			$kode_tmp=$h['kode'];
			if(mysql_num_rows(mysql_query("select * from pilihan where kode='".$kode."' and kode<>'".$kode_tmp."'"))>0){
				$error='Kode sudah terdaftar. Silahkan gunakan kode yang lain.';
			}else{
				$q="update pilihan set kode='".$kode."', nama='".$nama."' where id_pilihan='".$id."'";
				mysql_query($q);
				exit("<script>location.href='".$link_list."';</script>");
			}
		}
	...
	if($action=='delete'){
		$id=$_GET['id'];
		mysql_query("delete from pilihan where id_pilihan='".$id."'");
		exit("<script>location.href='".$link_list."';</script>");
	}
}
...
<h3 class="p2">Update Data Pilihan</h3>
...
<table class="table">
  ...
  <tr>
	<td>Pilihan<span class="required">*</span> </td>
